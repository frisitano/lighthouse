...
pub trait Eth2Enr {
    ...
    /// Whether this node has an execution proof engine configured and can serve execution proofs.
...
pub fn build_enr<E: EthSpec>(
    ...
    // advertise execution proof engine support if configured
...
fn compare_enr(local_enr: &Enr, disk_enr: &Enr) -> bool {
    ...
    (local_enr.ip6().is_none() || local_enr.ip6() == disk_enr.ip6())
        ...
        // PEERDAS_CUSTODY_GROUP_COUNT_ENR_KEY and EXECUTION_PROOF_ENR_KEY to match, otherwise we
        // use a new ENR. This will likely only be true for non-validating nodes.
